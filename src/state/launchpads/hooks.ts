...
const deserializeLaunchpad = (launchpad: SerializedLaunchpad): DeserializedLaunchpad => {
  const { id, isActive, isTombFork, isPrivateSale, address, name, subTitle, description, projectSiteUrl, releaseAt } =
    launchpad
  return {
    id,
    isActive,
...
export const useLaunchpads = (): { launchpads: DeserializedLaunchpad[]; useDataLoaded: boolean } => {
  return useSelector((state: State) => ({
    launchpads: state.launchpads.data.map(deserializeLaunchpad),
    useDataLoaded: state.launchpads.userDataLoaded,
  }))
}
