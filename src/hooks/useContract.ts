...
export const useLaunchpadContract = (address: string) => {
  const { data: signer } = useSigner()
  return useMemo(() => getLaunchpadContract(address, signer), [address, signer])
}
...
export const usePredictionsContract = (address: string, tokenSymbol: string) => {
  ...
  return useMemo(() => {
    ...
    const getPredContract = tokenSymbol === 'ROYAL' ? getCakePredictionsContract : getPredictionsContract
