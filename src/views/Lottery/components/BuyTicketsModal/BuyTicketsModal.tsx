...
const BuyTicketsModal: React.FC<React.PropsWithChildren<BuyTicketsModalProps>> = ({ onDismiss }) => {
  ...
  useEffect(() => {
    const getMaxPossiblePurchase = () => {
      ...
      // If the users' max ROYAL balance purchase is less than the contract limit - factor the discount logic into the max number of tickets they can purchase
  ...
  const getErrorMessage = () => {
    if (userNotEnoughCake) return t('Insufficient ROYAL balance')
    ...
    <StyledModal title={t('Buy Tickets')} onDismiss={onDismiss} headerBackground={theme.colors.gradientCardHeader}>
      ...
      <Flex flexDirection="column">
        <Flex mb="8px" justifyContent="space-between">
          ...
          <Text color="textSubtle" fontSize="14px">
            {priceTicketInCake && getFullDisplayBalance(priceTicketInCake.times(ticketsToBuy || 0))} ROYAL
          </Text>
        </Flex>
        <Flex mb="8px" justifyContent="space-between">
          <Flex>
