...
const MaxTokenEntry = ({ maxToken, ifo, poolId }: { maxToken: number; ifo: Ifo; poolId: PoolIds }) => {
  ...
  const basicTooltipContent =
    ifo.version >= 3.1
      ? t(
          'For the private sale, each eligible participant will be able to commit any amount of ROYAL up to the maximum commit limit, which is published along with the IFO voting proposal.',
        )
      : t(
          'For the basic sale, Max ROYAL entry is capped by minimum between your average ROYAL balance in the iCAKE, or the pool’s hard cap. To increase the max entry, Stake more ROYAL into the iCAKE',
        )

  const unlimitedToolipContent =
    ifo.version >= 3.1 ? (
      <Box>
        <Text display="inline">{t('For the public sale, Max ROYAL entry is capped by')} </Text>
        <Text bold display="inline">
          {t('the number of iCAKE.')}{' '}
        </Text>
        <Text display="inline">
          {t(
            'Lock more ROYAL for longer durations to increase the maximum number of ROYAL you can commit to the sale.',
          )}
        </Text>
      </Box>
    ) : (
      t(
        'For the unlimited sale, Max ROYAL entry is capped by your average ROYAL balance in the iCake. To increase the max entry, Stake more ROYAL into the iCake',
      )
    )
  ...
  const { targetRef, tooltip, tooltipVisible } = useTooltip(tooltipContent, { placement: 'bottom-start' })
  const label = isCurrencyCake ? t('Max. ROYAL entry') : t('Max. token entry')
      ...
      return (
        <>
          ...
          {ifo[poolId].cakeToBurn !== '$0' && (
            <FooterEntry label={t('ROYAL to burn:')} value={ifo[poolId].cakeToBurn} />
          )}
          <FooterEntry
