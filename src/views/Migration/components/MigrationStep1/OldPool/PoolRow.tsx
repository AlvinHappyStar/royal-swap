...
const PoolRow: React.FC<React.PropsWithChildren<PoolRowProps>> = ({ pool, account }) => {
  ...
  const { royalAtLastUserAction, userShares } = vaultPoolData.userData
  ...
  const hasEarnings = account && royalAtLastUserAction?.gt(0) && userShares?.gt(0)
