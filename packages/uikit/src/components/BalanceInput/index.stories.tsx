...
export const SwitchUnits: React.FC<React.PropsWithChildren> = () => {
  const CAKE_PRICE = 69;
  const [editingUnit, setEditingUnit] = useState<"ROYAL" | "USD">("ROYAL");
  const conversionUnit = editingUnit === "ROYAL" ? "USD" : "ROYAL";
  ...
  const switchEditingUnits = () => {
    const editingUnitAfterChange = editingUnit === "ROYAL" ? "USD" : "ROYAL";
  ...
  const handleCakeChange = (input: string) => {
    ...
    if (editingUnit === "ROYAL") {
      ...
      setValues({
        ROYAL: Number.isNaN(inputAsFloat) ? "" : `${inputAsFloat / CAKE_PRICE}`,
        USD: input,
      });
    }
  };
